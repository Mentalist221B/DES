...
int message[64], xor_arr[32], rk_arr[32], res_arr[32], res1[32];         //Array for storing block in binary format
char block[9], hexa[16], hex[16];              //Array for accepting plaintext block by block
...
void divide()                   //Divides the message_ip into left and right half
{
    int i;
...
void initial_permutation()                  //Produces an initial permutation of message
{
    int i;

    for(i=0; i<64; i++)
    {
        message_ip[i] = message[IP[i]-1];
    }
...
void final_permutation()
{
    ...
    for(i=0; i<64; i++)
    {
        if(IP_1[i] <= 32)
        {
            message[i] = R[0][IP_1[i]-1];
        }
        else
        {
            message[i] = L[0][IP_1[i]-33];
        }
    }
